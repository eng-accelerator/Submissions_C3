...
class DocumentProcessor:
    ...
    def _extract_credit_card_data(self, doc: Dict[str, Any]) -> List[Dict[str, Any]]:
        """Extract credit card data - FIXED VERSION"""
        data = []
        text = doc.get('full_text', '')
        
        # Patterns for credit card fields
        patterns = {
            'Total Amount Due': [
            ...
            for pattern in pattern_list:
                ...
                if matches:
                    ...
                    try:
                        ...
                        # Validation
                        if 0 < amount < 100000000:  # < 10 crore (reasonable for credit cards)
        ...
        # Convert to data entries
        for category, amount in extracted.items():
            data.append({
                'type': 'debt',
                'category': category,
                'amount': amount,
